...
def postprocess_simplified_network(json_routes, json_uris, json_found_json, lines_found):
    # read current list of (calculated) coordinates and lineStrings
    tmp_features = json_found_json
    lFeatures = lines_found
    ...
    while prev_not_found_len > len(final_not_found):
        ...
        for uri in not_found_uris:
            ...
            if (len(nei_degree) >= 2):
                ...
                # paths = list(p)
                # print("paths 1: ", p)
                distances = ggraph.find_path_distance(idx, paths[0][0],json_routes)
                ...
                # add the new coordinate to the existing featureCollection of points
                tmp_features['features'].append(geojson.Feature(geometry=geojson.Point((lat_lon[1], lat_lon[0])),
                                                                properties={'URI': uri, 'status': 'new2',
                                                                            'region': curr_uri_reg}))
                # creat linestrings for connecting start and end, atart/end to the new coordinate
                # lFeatures['features'].append(geojson.Feature(
                #     geometry=geojson.LineString([(float(startCoord[1]), float(startCoord[0])),
                #                                  (lat_lon[1], lat_lon[0])]),
                #     properties={}))
                # lFeatures['features'].append(geojson.Feature(
                #     geometry=geojson.LineString([(float(endCoord[1]), float(endCoord[0])),
                #                                  (lat_lon[1], lat_lon[0])]),
                #     properties={}))
                lFeatures['features'].append(geojson.Feature(
                    geometry=geojson.LineString([(float(startCoord[1]), float(startCoord[0])), (float(endCoord[1]),
                                                                                                float(endCoord[0]))]),
                    properties={}))
                new_coords[uri] = [lat_lon, curr_uri_reg]
                not_found_uris.remove(uri)
    return [tmp_features, lFeatures]