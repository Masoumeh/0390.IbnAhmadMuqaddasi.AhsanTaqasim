...
def extract_tri_route_WIt(input_file, out_file):
    """
    ...
    with open(input_file, "r", encoding="utf8") as f1:
        f1 = f1.read().split("\n")
        for l in f1:
            if l.startswith("#$#FRIT"):
                key = l = l.split("#$#")[1][4:].strip() + "_" + l.split("#$#")[2][4:].strip()
            ...
            if l.startswith("#$#FROM"):
                ...
                for v in vals:
                    tmp_tri = [l[0].strip() + "_" + key if key != "" else l[0] ,
                               l[1].strip() + "_" + key if key != "" else l[1], valTag + v]
                    data.append(tmp_tri)
...
def extract_iti_route(input_file, out_file):
    """
    To extract main itinerary sections data in triples from a tagged text.
    The output is a csv file, each line holding triples (FROM, TO, DIST)
    :param input_file: The given file
    :return:
    """
    ...
    with open(out_file, "w", encoding="utf8") as out_file:
        ...
        for d in data:
            print(d[0][4:].strip())
            print(d[1][4:].strip())
            topo_set.add(d[0][4:].strip())
            topo_set.add(d[1][4:].strip())
            # writer.writerow({'from': d[0][4:].strip(),
            #                  'to': d[1][4:].strip(),
            #                  'distance': d[-1][4:].strip()})
            # writer.writerow(d[0])
