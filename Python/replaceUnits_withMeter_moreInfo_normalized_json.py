...
def replaceUnitsWithMeter(fileName):
    ...
    for k in numbers:
      normalized_number[norm.normalizeArabic(k)] = numbers[k]
    for k in unit_distance:
      normalized_unit_distance[norm.normalizeArabic(k)] = unit_distance[k]
    for k in pluralUnits:
      normalized_pluralUnits[norm.normalizeArabic(k)] = pluralUnits[k]
    ...
    with open(fileName, 'r') as meterFile:
      ...
      for row in distReader:
          ...
          if len(splitDist) == 1:
            if splitDist[0] in unit_distance or norm.normalizeArabic(splitDist[0]) in normalized_unit_distance:
              meter = normalized_unit_distance[norm.normalizeArabic(splitDist[0])]
            ...
            if re.search('[0-9]', splitDist[0]):
              if norm.normalizeArabic(splitDist[1]) in normalized_unit_distance or splitDist[1] in unit_distance:
                meter = float(norm.normalizeArabic(splitDist[0])) * normalized_unit_distance[norm.normalizeArabic(splitDist[1])]
                distReader[row]['cornu_meter'] = meter
                routes[row] = distReader[row]
                
              elif norm.normalizeArabic(splitDist[1]) in normalized_pluralUnits or splitDist[1] in pluralUnits:
                single_unit = normalized_pluralUnits[norm.normalizeArabic(splitDist[1])]
                meter = float(norm.normalizeArabic(splitDist[0])) * normalized_unit_distance[single_unit]
                distReader[row]['cornu_meter'] = meter
                routes[row] = distReader[row]
              else:
                distReader[row]['cornu_meter'] = "null"
              ...
              if unit != None:
                ...
                if norm.normalizeArabic(value) in normalized_number or value in numbers: 
                  multiplyValue = normalized_number[norm.normalizeArabic(value)]
                  if norm.normalizeArabic(splitDist[unit_index]) in normalized_pluralUnits or splitDist[unit_index] in pluralUnits:
                    single_unit = normalized_pluralUnits[norm.normalizeArabic(splitDist[unit_index])]
                    meter = normalized_number[norm.normalizeArabic(value)] * normalized_unit_distance[single_unit]
                    distReader[row]['cornu_meter'] = meter
                    routes[row] = distReader[row]

                  elif norm.normalizeArabic(splitDist[unit_index]) in normalized_unit_distance or splitDist[unit_index] in unit_distance:
                    meter = normalized_number[norm.normalizeArabic(value)] * normalized_unit_distance[norm.normalizeArabic(splitDist[unit_index])]
                    distReader[row]['cornu_meter'] = meter
                    routes[row] = distReader[row]
